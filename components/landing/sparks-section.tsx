...
function SparkCard({ spark, index }: { spark: LightupSpark; index: number }) {
  return (
    <motion.div
      ...
      className={`group bg-[#111827]/80 border rounded-2xl p-5 sm:p-6 transition-all duration-300 cursor-pointer hover:-translate-y-1 ${
        spark.is_highlight
    ...
    >
      <div className="flex justify-between items-center mb-4 sm:mb-5">
        <span
          className={`text-[10px] sm:text-[11px] font-semibold tracking-wider uppercase px-2 sm:px-2.5 py-1 rounded-md ${
            spark.is_highlight
      ...
      <h3 className="text-base sm:text-lg font-bold text-slate-100 mb-3 sm:mb-4 leading-snug group-hover:text-[#D4AF37] transition-colors">
        {spark.title}
      </h3>

      <div className="flex justify-between items-center pt-3 sm:pt-4 border-t border-slate-800/40">
        <div className="flex items-center gap-1.5 text-[12px] sm:text-[13px] text-slate-500">
...
export default function SparksSection({ sparks }: SparksSectionProps) {
  return (
    <section id="sparks" className="py-10 sm:py-14 px-4 sm:px-6 bg-[#0D1220] relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 sm:mb-12 gap-4 sm:gap-6">
          ...
          >
            ...
            <p className="text-slate-500 mt-2 sm:mt-3 max-w-md text-[14px] sm:text-[15px] leading-relaxed">
              מאמרי &lsquo;ספארק 10&rsquo; - קריאה קצרה של עד 2 דקות שנותנת ערך, ידע
              וכלים לשילוב בין זהות, ערכים וקריירה.
            </p>
          </motion.div>
          <button className="text-slate-300 font-semibold flex items-center gap-1.5 text-[13px] sm:text-[14px] border border-slate-700 px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg hover:border-[#D4AF37]/40 hover:text-[#D4AF37] transition-all w-full sm:w-auto justify-center sm:justify-start">
