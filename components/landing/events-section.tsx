...
function EventCard({ event, index }: { event: LightupEvent; index: number }) {
  return (
    <motion.div
      ...
      className={`group flex gap-5 items-center p-5 rounded-xl border transition-all duration-300 cursor-pointer hover:-translate-y-0.5 ${
        event.is_special
  ...
  return (
    <section id="events" className="py-14 px-6 bg-[#0B0F1A] relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#D4AF37]/10 text-[#D4AF37] text-[12px] font-semibold tracking-wide uppercase mb-4">
            <Calendar size={14} />
            לוח אירועים
          </div>
